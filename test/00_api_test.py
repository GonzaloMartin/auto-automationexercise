...
@allure.story("Buscar los productos especificados")
@allure.description("Función que me permite buscar todos los productos de la API.")
@allure.severity(allure.severity_level.CRITICAL)
def test_buscar_productos():
    with allure.step("Realizar Solicitud"):
        response = srv_buscar_productos(item="Sleeves Printed Top - White")
        allure.attach(json.dumps(response[1], indent=4), name="Respuesta del servicio", attachment_type=allure.attachment_type.JSON)
    ...
    with allure.step("Validar si Existe al manos una coincidencia de búsqueda"):
        item = response[1].get("products")
        if item is not None:
            assert len(item) > 0, "El accessToken está vacío."
        else:
